...
public interface AdminRepository extends UserRepository<Admin> {
    ...
    @Query("SELECT a FROM Admin a WHERE (:username IS NULL OR a.username like %:username%) AND (:firstName IS NULL OR a.firstName like %:firstName%) AND (:lastName IS NULL OR a.lastName like %:lastName%) AND (:status IS NULL OR a.status =:status)")
